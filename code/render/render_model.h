...
// GLTF / cgltf loader (Step 2+). Külön fájlban a render_model_gltf.h-ban,
// a model_from_mem dispatch-er hívja a model_from_mem_gltf-et.
#include "render_model_gltf.h"
...
model_t model_from_mem(const void *mem, int len, int flags) {
    ...
    int error = 1;
    if( ptr && len >= 4 ) {
        // ---- Magic-dispatch: IQM vs GLB ("glTF") vs JSON ('{' nyitóbrace).
        // A GLTF-loader jelenleg stub — Step 1 csak detektál + log.
        if( len >= (int)sizeof(struct iqmheader) &&
            !memcmp(ptr, IQM_MAGIC, sizeof(((struct iqmheader*)0)->magic)) ) {
        ...
        } else if( !memcmp(ptr, "glTF", 4) ) {
            // GLB binary (magic = 0x46546C67 = "glTF").
            error = model_from_mem_gltf(&m, q, mem, len, flags);
        } else {
            // Try JSON .gltf — skip whitespace, check first non-ws char is '{'.
            int i = 0;
...
model_t model(const char *filename, int flags) {
    ...
    if (m) {
        ...
        // Ha a betöltés fail-elt (m.iqm == NULL — pl. nem-támogatott formátum,
        // header-error), NE cache-eljük és NE hívjunk shader2_dup-ot, mert az
        // üres `shaderinfo[]` STRDUP(NULL)-on crashelne. Csak return üres
        // model_t-t — a hívó (editor render-walk) `mt.iqm == NULL` check-jén
        // fennakad és failed-paths-be teszi.
        if (!new_model.iqm) return new_model;
