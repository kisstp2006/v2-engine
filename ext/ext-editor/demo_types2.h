...
void *cam2_find(cam2 *obj, int prop) {
    if( prop == 'xfrm' ) return obj->cam_.view;
    if( prop == 'icon' ) return UI_ICON(PHOTO_CAMERA);
    return NULL;
}
AUTORUN {
    STRUCT(cam2, int, dummy); // @fixme: remove me
    EXTEND(cam2, ctor,tick,draw,find,view);
}
...
void *model2_find(model2 *obj, int prop) {
    if( prop == 'xfrm' ) return obj->mdl_.pivot;
    if( prop == 'icon' ) return UI_ICON(GAMEPAD);
    return NULL;
}
AUTORUN {
    STRUCT(model2, char*, filename_);
    STRUCT(model2, mat44, mdl_.pivot);
    EXTEND(model2, ctor,edit,draw,tick,find);
}
...
#endif
void *lit_find(lit *obj, int prop) {
...
int lit_edit(lit *obj) {
    ...
    const char *icon = (const char *)lit_find(obj,'icon');
    editor_label2(vec2(obj->pos.x,obj->pos.y),icon);
...
void* kid_find(kid *obj, int prop) {
    if( prop == 'icon' ) return UI_ICON(IMAGE); // ACCOUNT_BOX);
    return NULL;
}
...
AUTORUN {
    ...
    EXTEND(kid, ctor,tick,draw,edit,find); // ,aabb);
}
